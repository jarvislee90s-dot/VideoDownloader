...
基于 [yt-dlp](https://github.com/yt-dlp/yt-dlp) 与 curl_cffi，支持通用站点、加密 HLS 流、**B站（bilibili.com）**、**微信视频号（channels.weixin.qq.com）**以及**微信公众号文章内嵌视频（mp.weixin.qq.com）**。日常使用只需打开网页操作。
...
- **任务卡片**：显示视频名、时长、总大小、状态、进度条。
  - 等待中：加入队列后即异步获取标题、时长、总大小；可点击 ⏸ 暂停，或 ✕ 删除。
  - 下载中：显示实时进度、速度、预计剩余时间，可暂停。
  - 已暂停：点击 ▶ 继续。
  - 已完成：点击 ▶ 用系统默认播放器播放视频。
  - 失败：显示 ↻ 重试。
- **分组折叠**：等待中 / 下载中 / 已暂停 / 已完成 / 失败 自动分组，可点击折叠。

> 大小在下载中显示为估算值；下载完成后会显示磁盘上的精确大小。
...
- **等待中即显信息**：任务加入队列后异步拉取元数据，无需等到开始下载。
- **播放已完成视频**：已完成任务点击 ▶ 用系统默认播放器直接播放。
...
│   ├── mp_article.py          # 公众号文章内嵌视频下载（Playwright 劫持媒体流）
...
- **B站（bilibili.com）下载失败 / 412 / v_voucher**：B站对 yt-dlp 有风控。工具已内置 curl_cffi 绕过 412；若仍提示风控，请确保浏览器已登录 B站账号（默认读取 Chrome 登录态 cookies）。
...
- **微信视频号/公众号视频提示"未检测到视频号登录态"**：视频号需要登录态。在项目目录运行 `python wechat_login.py`，用微信扫码登录一次（登录状态保存在本地 `profile_browser/`，之后无需再扫），然后点任务重试 ↻。注意：部分视频号内容被发布者设置为仅微信内观看，网页端拿不到视频地址，此类内容暂不支持；公众号文章内嵌视频无需登录即可下载。加密视频解密失败时会保留 `.encrypted` 密文文件，请携带链接反馈。
